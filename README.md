...
### Banco PAN no Claude, ChatGPT e agentes de IA via Open Finance

Conecte sua conta do **Banco PAN** ao Claude, ChatGPT e agentes de IA via Open Finance Brasil. Pergunte sobre saldos, extratos, faturas de cartão e investimentos do Banco PAN em linguagem natural. Regulado pelo Banco Central, somente leitura.

- 🏦 **Banco PAN via Open Finance Brasil**, regulado pelo Banco Central
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `Banco PAN MCP` e **URL** `https://api.mcp.ai/p_pan`.

### Cursor

[➕ Instalar Banco PAN MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=pan&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9wYW4ifQ==)

### VS Code (Copilot Chat)

[➕ Instalar Banco PAN MCP no VS Code](vscode:mcp/install?name=pan&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_pan%22%7D)
...
```
Quanto gastei no cartão do Banco PAN esse mês?
Resuma minha fatura do Banco PAN em aberto
Qual meu saldo na conta do Banco PAN?
Como está minha carteira de investimentos no Banco PAN?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta Banco PAN com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o Banco PAN?**
Este MCP é focado no Banco PAN. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [pan@mcp.ai](mailto:pan@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/pan-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_pan` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
